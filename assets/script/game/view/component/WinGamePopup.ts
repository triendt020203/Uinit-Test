...
import { ResultEntryWinGame, ResultSpades } from "../../entities/ResultSpades";

const { ccclass, property } = cc._decorator;

@ccclass
export default class WinGamePopup extends cc.Component {
    ...
    updateData(data: ResultSpades): void {
        for (let i = 0; i < data.winPlayers.length; i++) {
            data.winPlayers.sort((a, b) => b.point - a.point);
        }
        for (let i = 0; i < data.winPlayers.length; i++) {
            let entry: ResultEntryWinGame = data.winPlayers[i];
            let seat = this.gameScene.seatManager.getSeatByGuserid(entry.guserid);
            this.players[i].updateWinResult(data.winPlayers[i]);
