...
export default class PositionHelper {
    ...
    public static setWhiteWinPos(node: cc.Node, whiteWin: cc.Node) {
        if (node.x < 0) {
            whiteWin.x = node.x + 25 + whiteWin.width / 2;
            whiteWin.y = node.y + 35;
        } else {
            whiteWin.x = node.x - 30 - whiteWin.width / 2;
            whiteWin.y = node.y + 35;
        }
    }

    /**
     * Create 6 positions for cards
     * 
     * @param cardControl 
     */
    public static createCardPositions(cardControl: CardControl): void {
    ...
    /**
     * Calculating position of dump cards on the tables     
     */
    public static getDumpPosition(idx: number, size?: number): any {
        switch (idx) {
            ...
            case 4:
                return this.getDumpPosition4(size);
        }
        return null;
    }
    ...
    private static getDumpPosition4(size: number): any {
        const SPACE_X = 115;
        const cardW = CardSize.TABLE_WIDTH;
        const cardH = CardSize.TABLE_HEIGHT;
        let angles = [];
